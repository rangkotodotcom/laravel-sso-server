...
<head>
    ...
    <title>Izinkan Akses Aplikasi - {{ $client->name }}</title>
...
<body
    ...
    <div class="max-w-md w-full space-y-6" role="main" aria-label="Halaman otorisasi aplikasi">
        <div class="max-w-md w-full bg-white dark:bg-gray-800 p-6 rounded-lg shadow space-y-6">
            <div class="text-center">
                ...
                <h1 class="text-2xl font-bold">Izinkan Akses Aplikasi</h1>
                <p>Anda sedang diminta untuk memberikan akses aplikasi <strong>{{ $client->name }}</strong>.</p>
            </div>
            ...
            <div class="bg-yellow-100 dark:bg-yellow-800 text-yellow-800 dark:text-yellow-300 p-4 rounded-lg shadow">
                <strong>⚠️ Perhatian:</strong> Pastikan Anda hanya memberikan akses ke aplikasi yang Anda percaya.
            ...
            <ul class="list-disc list-inside text-sm text-gray-700 dark:text-gray-300 space-y-2"
                aria-label="Daftar izin yang diminta aplikasi">
                @foreach ($scopes as $scope)
                    <li>
                        ...
                        <p class="text-xs text-gray-600 dark:text-gray-400 mt-0.5">
                            ...
                            @else
                                Izin terkait akses data lainnya yang diperlukan aplikasi.
                            @endif
                        </p>
                    </li>
                @endforeach
            ...
            <div class="flex gap-4 mt-4">
                ...
                <form method="post" action="{{ route('passport.authorizations.approve') }}" class="flex-1"
                    aria-label="Form untuk mengizinkan aplikasi">
                    @csrf
                    ...
                    <button type="submit"
                        ...
                        aria-label="Tombol izinkan akses aplikasi">
                        Izinkan
                    </button>
                </form>
                ...
                <form method="post" action="{{ route('passport.authorizations.deny') }}" class="flex-1"
                    aria-label="Form untuk menolak akses aplikasi">
                    @method('delete')
                    @csrf
                    <input type="hidden" name="state" value="{{ request('state') }}" />
                    <input type="hidden" name="client_id" value="{{ $client->id }}" />
                    ...
                    <button type="submit"
                        ...
                        aria-label="Tombol batalkan akses aplikasi">
                        Batal
                    </button>
                </form>
            </div>
            ...
            <div class="mt-4 text-center">
                <a href="{{ route('logout') }}"
                    ...
                    aria-label="Keluar dari aplikasi">Logout</a> |
                <a href="{{ url('/') }}"
                    class="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 underline"
                    aria-label="Kembali ke beranda">Kembali ke Beranda</a>
            </div>
        </div>
